...
class PublicGoodsServer {
  ...
  async sendPayment(address, amount, destinTag, callback){
    ...
    const secret = process.env.PUBLICGOODS_MINTER_WALLET_SEED
    //const source = process.env.PUBLICGOODS_MINTER_WALLET
  ...
  async mintNFT(uri: string, address: string){
    ...
    const contract = process.env.NEXT_PUBLIC_PUBLICGOODS_MINTER_CONTRACT
...
const PublicGoods = new PublicGoodsServer()
